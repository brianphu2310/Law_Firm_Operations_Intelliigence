...
with st.sidebar:
    ...
    st.markdown(
        '<div style="height:1px; background:rgba(255,255,255,0.15); margin:14px 0 12px;"></div>'
        '<div class="sidebar-label" style="margin-bottom:8px;">Annual cost — all platforms</div>',
        unsafe_allow_html=True,
    )
    ...
    order = sorted(range(len(platform_names)), key=lambda i: platform_costs[i])
    SIDEBAR_SHORT_NAMES = {
    ...
    sorted_names = [SIDEBAR_SHORT_NAMES.get(platform_names[i], platform_names[i]) for i in order]
    sorted_costs = [platform_costs[i] for i in order]
    bar_colors = [TEAL if platform_names[i] == selected_platform else "rgba(255,255,255,0.25)" for i in order]

    fig_side = go.Figure(go.Bar(
        x=sorted_costs, y=sorted_names, orientation="h",
        marker_color=bar_colors,
        text=[fmt_money(c) for c in sorted_costs], textposition="outside",
        textfont=dict(color="#ffffff", size=9),
        hovertemplate="%{y}: %{text}<extra></extra>",
    ))
    fig_side.update_layout(
        margin=dict(l=4, r=26, t=4, b=4), height=240,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        showlegend=False, font=dict(family="Inter, sans-serif", color="rgba(255,255,255,0.75)", size=9),
    )
    fig_side.update_xaxes(visible=False)
    fig_side.update_yaxes(tickfont=dict(size=9, color="rgba(255,255,255,0.85)"))
    st.plotly_chart(fig_side, width="stretch", config={"displayModeBar": False})
    st.markdown(
        '<div style="font-size:10px; color:rgba(255,255,255,0.45); margin-top:2px;">Teal bar = currently selected platform</div>',
        unsafe_allow_html=True,
    )
    ...
    with col_matrix:
        with card():
            st.markdown('<div class="card-title">Attorney Utilization — Hours by Practice Area</div>',
                        unsafe_allow_html=True)
            st.caption("Click an attorney to zoom into their practice-area split. Click center to zoom back out.")
            ...
            fig_matrix.update_layout(
                margin=dict(l=4, r=4, t=28, b=4), height=230,
