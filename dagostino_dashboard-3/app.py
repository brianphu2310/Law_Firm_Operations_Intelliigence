...
def branch_globe_fig(height=230):
    """Interactive orthographic 'globe' — Australia highlighted, branch pins on top.
    Plotly's orthographic geo projection supports native click-drag rotation."""
    fig = go.Figure()
    fig.add_trace(go.Choropleth(
        locations=["AUS"], locationmode="ISO-3", z=[1],
        colorscale=[[0, TEAL], [1, TEAL]], showscale=False,
        marker_line_color="#ffffff", marker_line_width=0.6,
        hovertemplate="Australia<extra></extra>",
    ))
    bdf = pd.DataFrame(BRANCHES)
    fig.add_trace(go.Scattergeo(
        lat=bdf["lat"], lon=bdf["lon"], mode="markers",
        marker=dict(size=9, color=INK, line=dict(color="#ffffff", width=1.4)),
        text=bdf["name"], hovertemplate="%{text}<extra></extra>",
    ))
    fig.update_geos(
        projection_type="orthographic",
        projection_rotation=dict(lon=134, lat=-26, roll=0),
        showland=True, landcolor="#dce8ee",
        showocean=True, oceancolor="#eef4f8",
        showcountries=True, countrycolor="#c9d6dd",
...
def render_overview():
    ...
    spacer()
    col_trend, col_donut, col_map = st.columns([1.7, 1.2, 0.8])
    ...
    with col_map:
        st.markdown('<div class="card-title" style="padding:0 2px;">Branch Locations</div>',
                    unsafe_allow_html=True)
        st.plotly_chart(branch_globe_fig(height=230), width="stretch",
...
def _locations_tab():
    st.markdown('<div class="section-sub">3 branches — drag to rotate the globe, hover a pin, or check the cards below.</div>',
                unsafe_allow_html=True)
    st.plotly_chart(branch_globe_fig(height=340), width="stretch",
                     config={"displayModeBar": False, "scrollZoom": False})
