...
def branch_globe_fig(height=230):
    ...
    fig.add_trace(go.Choropleth(
        ...
        colorscale=[
            [0.0, "#eaf5f7"], [0.15, "#cfe9ec"], [0.35, "#8fd0d6"],
            [0.55, "#4fb0b8"], [0.75, "#1f8f97"], [1.0, "#0a5c63"],
        ],
        showscale=False,
    ...
    fig.update_geos(
        ...
        projection_rotation=dict(lon=134, lat=-26, roll=0),
        showland=True, landcolor="#cfe0ea",
        showocean=True, oceancolor="#e3eef5",
        showcountries=True, countrycolor="#a9c0d0",
        showcoastlines=True, coastlinecolor="#9db3c2", coastlinewidth=0.6,
        showlakes=True, lakecolor="#e3eef5",
        showframe=False,
        lonaxis=dict(showgrid=True, gridcolor="#d7e3ec", gridwidth=0.5),
        lataxis=dict(showgrid=True, gridcolor="#d7e3ec", gridwidth=0.5),
...
def render_overview():
    ...
    with col_country:
        with st.container(border=True):
            top1, top2 = st.columns([1.5, 1], vertical_alignment="center")
            with top1:
                st.markdown('<div class="card-title">Matters by Country</div>', unsafe_allow_html=True)
            with top2:
                mdf_hdr = pd.DataFrame(MATTERS)
                st.markdown(
                    f'<div style="text-align:right; font-size:10.5px; color:{MUTED};">'
                    f'{mdf_hdr["country"].nunique()} countries</div>',
                    unsafe_allow_html=True,
                )
            mdf = pd.DataFrame(MATTERS)
            by_country = mdf.groupby("country").agg(matters=("name", "count"), value=("value", "sum")).reset_index()
            by_country = by_country.sort_values("matters", ascending=True)
            fig_c = go.Figure(go.Bar(
                x=by_country["matters"], y=by_country["country"], orientation="h",
                marker_color=[TEAL if c == "Australia" else "#67c1cb" for c in by_country["country"]],
                customdata=by_country["value"],
                hovertemplate="%{y}: %{x} matters ($%{customdata:,.0f})<extra></extra>",
            ))
            layout_c = plotly_base(210)
            layout_c["margin"] = dict(l=8, r=16, t=4, b=4)
            fig_c.update_layout(**layout_c)
            fig_c.update_xaxes(showgrid=True, gridcolor="#f1f5f9", tickfont=dict(size=8.5))
            fig_c.update_yaxes(tickfont=dict(size=9.5, color=MUTED))
