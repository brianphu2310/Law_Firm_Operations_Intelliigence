...
with st.sidebar:
    ...
    fig_side.update_layout(
        margin=dict(l=18, r=18, t=18, b=18), height=230,
        ...
        polar=dict(
            ...
            angularaxis=dict(gridcolor="rgba(255,255,255,0.12)", linecolor="rgba(255,255,255,0.2)",
                              tickfont=dict(size=9, color="rgba(255,255,255,0.85)")),
        ),
        showlegend=False,
    )
    ...
    if poly_event and poly_event.selection and poly_event.selection.points:
        ...
        if clicked in short_to_full and short_to_full[clicked] != st.session_state.sidebar_platform:
            ...
            st.rerun()
    st.markdown(
        '<div style="font-size:10px; color:rgba(255,255,255,0.45); margin-top:2px;">Longer wedge = higher annual cost · teal = selected</div>'
...
def render_overview():
    ...
    with card():
        pc1, pc2, pc3, pc4, pc5 = st.columns([1.3, 0.85, 0.85, 1.0, 1.1])
        with pc1:
            st.markdown(
        ...
        with pc5:
            ...
            fig_radar.add_trace(go.Scatterpolar(
                ...
                line=dict(color="#94a3b8", width=1.5), name="Current",
                hovertemplate="%{theta}: %{r}<extra>Current</extra>",
            ))
            fig_radar.add_trace(go.Scatterpolar(
                r=sel_scores + [sel_scores[0]], theta=score_cats + [score_cats[0]],
                fill="toself", fillcolor="rgba(10,132,150,0.25)",
                line=dict(color=TEAL, width=2), name="Selected",
                hovertemplate="%{theta}: %{r}<extra>Selected</extra>",
            ))
            fig_radar.update_layout(
                margin=dict(l=18, r=18, t=6, b=6), height=140,
                paper_bgcolor="rgba(0,0,0,0)",
                font=dict(family="Inter, sans-serif", color=MUTED, size=8),
                polar=dict(
                    bgcolor="rgba(0,0,0,0)",
                    radialaxis=dict(visible=True, showticklabels=False, ticks="", range=[0, 100],
                                     gridcolor="#eef2f3", linecolor="#eef2f3"),
                    angularaxis=dict(gridcolor="#eef2f3", linecolor="#eef2f3", tickfont=dict(size=8, color=MUTED)),
                ),
                showlegend=False,
            )
