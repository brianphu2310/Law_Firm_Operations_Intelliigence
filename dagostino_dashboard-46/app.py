...
def render_overview():
    ...
    with card():
        pc1, pc2, pc3, pc4, pc5 = st.columns([1.45, 0.8, 0.8, 0.95, 1.3])
        with pc1:
            badges_html = "".join(
                f'<div style="width:26px; height:26px; border-radius:7px; background:{color}; '
                f'color:#ffffff; font-size:9px; font-weight:700; display:flex; align-items:center; '
                f'justify-content:center; flex-shrink:0;">{initial}</div>'
                for initial, color in PLATFORM_BADGES.get(sel_name, [])
            )
            if is_baseline:
                advice = "This is your current setup — pick another option in the sidebar to compare cost and capability trade-offs."
            elif delta >= 0:
                advice = f"Could save {fmt_money(delta)}/yr vs your current setup — check the radar chart for capability trade-offs before switching."
            else:
                advice = f"Costs {fmt_money(-delta)}/yr more than your current setup — see the radar chart to weigh whether the capability gains justify it."
            st.markdown(
                f'<div style="display:flex; align-items:center; gap:6px; margin-bottom:3px;">'
                f'{badges_html}<div class="card-title" style="margin-bottom:0;">Platform: {sel_name}</div></div>'
                f'<div style="font-size:11.5px; color:{MUTED}; line-height:1.35;">{advice}</div>',
                unsafe_allow_html=True,
            )
        with pc2:
        ...
        with pc5:
            score_cats = ["Speed", "Cost", "Reliability", "Support", "Integration"]
            score_colors = ["#0a8496", "#f2b134", "#e2665a", "#6b46c1", "#3182ce"]
            baseline_scores = [baseline_p["scores"][c] for c in score_cats]
            sel_scores = [sel_p["scores"][c] for c in score_cats]
            fig_radar = go.Figure()
            fig_radar.add_trace(go.Scatterpolar(
                r=baseline_scores + [baseline_scores[0]], theta=score_cats + [score_cats[0]],
                fill="toself", fillcolor="rgba(148,163,184,0.18)", mode="lines+markers",
                line=dict(color="#94a3b8", width=1.75), marker=dict(size=4, color="#94a3b8"), name="Current",
                hovertemplate="%{theta}: %{r}<extra>Current</extra>",
            ))
            fig_radar.add_trace(go.Scatterpolar(
                r=sel_scores + [sel_scores[0]], theta=score_cats + [score_cats[0]],
                fill="toself", fillcolor="rgba(10,132,150,0.30)", mode="lines+markers",
                line=dict(color=TEAL, width=2.25), marker=dict(size=5, color=TEAL), name="Selected",
                hovertemplate="%{theta}: %{r}<extra>Selected</extra>",
            ))
            fig_radar.update_layout(
                margin=dict(l=8, r=8, t=6, b=6), height=125,
                paper_bgcolor="rgba(0,0,0,0)",
                font=dict(family="Inter, sans-serif", color=MUTED, size=7.5),
                polar=dict(
                    bgcolor="rgba(10,132,150,0.02)",
                    radialaxis=dict(visible=True, showticklabels=True, tickvals=[50, 100],
                                     tickfont=dict(size=6.5, color="#c2cbcd"), ticks="",
                                     gridcolor="#e4edee", linecolor="#e4edee", range=[0, 100]),
                    angularaxis=dict(gridcolor="#e4edee", linecolor="#e4edee", showticklabels=False, ticks=""),
                ),
                showlegend=False,
            )
            radar_col, radar_legend_col = st.columns([1.5, 1])
            with radar_col:
                st.markdown(
                    f'<div style="display:flex; gap:10px; font-size:9px; color:{MUTED}; justify-content:center; margin-bottom:-4px;">'
                    f'<span><span style="color:#94a3b8;">●</span> Current</span>'
                    f'<span><span style="color:{TEAL};">●</span> Selected</span></div>',
                    unsafe_allow_html=True,
                )
                st.plotly_chart(fig_radar, width="stretch", config={"displayModeBar": False})
            with radar_legend_col:
                legend_html = "".join(
                    f'<div style="display:flex; align-items:center; gap:4px; font-size:9.5px; color:{MUTED}; margin-bottom:3px;">'
                    f'<span style="width:6px; height:6px; border-radius:999px; flex-shrink:0; background:{score_colors[i]};"></span>'
                    f'{cat}</div>'
                    for i, cat in enumerate(score_cats)
                )
                st.markdown(f'<div style="padding-top:6px;">{legend_html}</div>', unsafe_allow_html=True)
