...
with st.container(key="topbar"):
    ...
    with nav_wrap:
        for col, item in zip(st.columns(len(NAV_ITEMS)), NAV_ITEMS):
            with col:
                if st.button(item, key=f"nav_{item}", type="primary" if (ss.view_kind == "page" and ss.view_name == item) else "secondary", width="stretch"):
