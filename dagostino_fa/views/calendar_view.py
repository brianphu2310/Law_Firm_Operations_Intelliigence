...
.cal-more { font-size:9px; color:#64748b; margin-top:1px; }
            ...
            ui.spacer(14)
            st.markdown(f'<div class="tbl-note">{len(evs)} dates in {sel:%B %Y}. Red = hearing/trial, amber = filing deadline, purple = mediation, teal = other. Today is outlined.</div>', unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        with c2:
            with ui.card("cal4"):
                ...
                d60 = up[up["days"] <= 60]
                g = d60.groupby(["attorney", "type"]).size().unstack(fill_value=0)
                g = g.loc[g.sum(1).sort_values().index]
                fig = go.Figure([go.Bar(y=g.index, x=g[c], orientation="h", name=c) for c in g.columns])
                fig.update_layout(**ui.base_layout(260, legend=True), barmode="stack")
