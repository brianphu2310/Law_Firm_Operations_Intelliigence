...
locations = pd.DataFrame([
    {"location_id": "LOC01", "location_name": "Sydney CBD", "state": "NSW", "city": "Sydney"},
    {"location_id": "LOC02", "location_name": "Parramatta", "state": "NSW", "city": "Parramatta"},
    {"location_id": "LOC03", "location_name": "Melbourne CBD", "state": "VIC", "city": "Melbourne"},
    {"location_id": "LOC04", "location_name": "Brisbane", "state": "QLD", "city": "Brisbane"},
    {"location_id": "LOC05", "location_name": "Perth", "state": "WA", "city": "Perth"},
])
...
counsel = pd.DataFrame({
    ...
    "chambers": rng.choice(["Banco Chambers", "Wentworth Chambers", "Owen Dixon Chambers",
                             "Level Twenty Seven", "Francis Burt Chambers"], len(counsel_names)),
    "specialty": rng.choice(["Litigation", "Commercial", "Property", "Family", "Insolvency"], len(counsel_names)),
})
...
for i in range(n_clients):
    ...
    client_rows.append({
        ...
        "client_name": name,
        "client_type": ctype,
        "location_id": rng.choice(locations.location_id, p=[0.35, 0.15, 0.25, 0.15, 0.10]),
    })
clients = pd.DataFrame(client_rows)
clients.to_csv(OUT / "clients.csv", index=False)

# ---------------------------------------------------------------- matters
matter_types = ["Conveyancing", "Commercial Litigation", "Family Law", "Wills & Estates",
                 "Commercial Advisory", "Insolvency", "Employment"]
matter_type_p = [0.28, 0.18, 0.16, 0.14, 0.10, 0.08, 0.06]
n_matters = 650
...
for i in range(n_matters):
    ...
    uses_counsel = mtype in ("Commercial Litigation", "Insolvency", "Employment") and rng.random() < 0.55
...
for _, m in matters.iterrows():
    n_txn = rng.integers(1, 6) if m.matter_type == "Conveyancing" else rng.integers(0, 4)
    running_balance = 0.0
...
infotrack_rows = []
txn_id = 1
conv_matters = matters[matters.matter_type == "Conveyancing"]
