...
.brand-name {{ font-weight:700; font-size:16px; color:#ffffff; }}
...
.avatar-initials {{
    ...
    color:{TEAL}; font-weight:700; font-size:12px;
    display:flex; align-items:center; justify-content:center; flex-shrink:0;
}}
.user-name {{ font-weight:600; font-size:13px; color:#ffffff; line-height:1.2; }}
.user-role {{ font-size:10px; color:rgba(255,255,255,0.75); line-height:1.2; }}
...
.kpi-card {{ background:{CARD}; border:1px solid {BORDER}; border-radius:14px; padding:5px 14px; height:100%; box-shadow: 0 10px 26px rgba(15,23,42,0.18), 0 3px 7px rgba(15,23,42,0.11); margin-bottom:28px; }}
...
.kpi-trend {{ display:flex; align-items:center; gap:4px; margin-top:0px; }}
...
with st.sidebar:
    st.markdown(
        f'<div style="font-size:13px; font-weight:700; color:{INK}; margin-bottom:2px;">Practice &amp; search platform</div>'
        f'<div style="font-size:11.5px; color:{MUTED}; margin-bottom:10px;">Switch to see the cost impact live on Overview.</div>',
...
def render_overview():
    ...
    with col_country:
        with card():
            st.markdown(
                '<div class="card-title" style="font-size:14.5px; text-align:center; color:#152b4e;">'
                'Revenue by Practice Area &amp; Payment Method</div>',
                unsafe_allow_html=True,
            )
            legend_items = "".join(
