...
.stApp {{ background: {BG}; }}
.block-container {{ padding-top: 0.5rem; padding-bottom: 0.6rem; max-width: 1440px; }}
section[data-testid="stSidebar"] {{ width: 230px !important; min-width: 230px !important; background: {TEAL} !important; }}
section[data-testid="stSidebar"] > div {{ padding-top: 1rem; }}
section[data-testid="stSidebar"] p, section[data-testid="stSidebar"] label,
section[data-testid="stSidebar"] span, section[data-testid="stSidebar"] div {{ color: #ffffff !important; }}
section[data-testid="stSidebar"] [data-testid="stCaptionContainer"] p {{ color: rgba(255,255,255,0.75) !important; }}
section[data-testid="stSidebar"] [role="radiogroup"] label {{
    background: rgba(255,255,255,0.08); border-radius: 10px; padding: 8px 10px; margin-bottom: 4px;
}}
section[data-testid="stSidebar"] [role="radiogroup"] label:hover {{ background: rgba(255,255,255,0.16); }}
section[data-testid="stSidebar"] [data-baseweb="radio"] div:first-child {{
    border-color: #ffffff !important;
}}
section[data-testid="stSidebar"] hr {{ border-color: rgba(255,255,255,0.2) !important; }}
.sidebar-brand {{ display:flex; align-items:center; gap:8px; margin-bottom:18px; padding: 0 4px; }}
.sidebar-brand-badge {{
    background:#ffffff; width:28px; height:28px; border-radius:8px;
    display:flex; align-items:center; justify-content:center;
    color:{TEAL}; font-weight:800; font-size:16px; flex-shrink:0;
}}
section[data-testid="stSidebar"] .sidebar-brand-badge {{ color:{TEAL} !important; }}
.sidebar-brand-name {{ font-weight:700; font-size:14px; color:#ffffff !important; }}
...
div[class*="st-key-cardblock_"] {{
    ...
    border: 1px solid #dbe6ee !important; padding: 1rem !important;
}}
...
.st-key-topbar {{
    background:{TEAL} !important; border-radius:18px !important; padding:14px 18px !important;
    box-shadow: 0 10px 28px rgba(15,23,42,0.20), 0 3px 8px rgba(15,23,42,0.12) !important;
}}
...
def branch_globe_fig(height=230):
    ...
    max_m = bdf["active_matters"].max()
    marker_sizes = 14 + (bdf["active_matters"] / max_m) * 16
    fig.add_trace(go.Scattergeo(
        lat=bdf["lat"], lon=bdf["lon"], mode="markers+text",
        marker=dict(size=marker_sizes, color=INK, line=dict(color="#ffffff", width=1.6)),
        text=bdf["active_matters"].astype(str), textposition="middle center",
        textfont=dict(size=9, color="#ffffff", family="Inter, sans-serif"),
        customdata=bdf[["name", "active_matters"]],
        hovertemplate="%{customdata[0]}: %{customdata[1]} matters<extra></extra>",
    ))
    fig.update_geos(
...
with st.sidebar:
    ...
    st.markdown(
        '<div style="font-size:13px; font-weight:700; margin-bottom:2px;">Practice &amp; search platform</div>'
        '<div style="font-size:11.5px; opacity:0.85; margin-bottom:10px;">Switch to see the cost impact live on Overview.</div>',
        unsafe_allow_html=True,
    )
    selected_platform = st.radio(
        "Platform", list(PLATFORM_OPTIONS.keys()),
        key="sidebar_platform", label_visibility="collapsed",
    )
    p = PLATFORM_OPTIONS[selected_platform]
    st.caption(f"${p['sub']}/seat/mo · ${p['fee']}/search avg")
...
with st.container(key="topbar"):
    brand_col, nav_cols_wrap, user_col = st.columns([1.15, 5.15, 1.3], vertical_alignment="center")
    with brand_col:
        st.markdown(
...
def render_overview():
    ...
    spacer(10)
    baseline_p = PLATFORM_OPTIONS[BASELINE_PLATFORM]
    sel_p = PLATFORM_OPTIONS[st.session_state.get("sidebar_platform", BASELINE_PLATFORM)]
    sel_name = st.session_state.get("sidebar_platform", BASELINE_PLATFORM)
    baseline_annual = (baseline_p["sub"] * baseline_p["seats"] + baseline_p["fee"] * baseline_p["searches"]) * 12
    sel_annual = (sel_p["sub"] * sel_p["seats"] + sel_p["fee"] * sel_p["searches"]) * 12
    delta = baseline_annual - sel_annual
    is_baseline = sel_name == BASELINE_PLATFORM

    with card():
        pc1, pc2, pc3, pc4 = st.columns([1.6, 1, 1, 1.2])
        with pc1:
            st.markdown(
                f'<div class="card-title">Platform: {sel_name}</div>'
                f'<div style="font-size:11.5px; color:{MUTED};">Chosen in the sidebar — switch it there to compare live.</div>',
                unsafe_allow_html=True,
            )
        with pc2:
            st.markdown(plain_kpi_html("Annual cost", fmt_money(sel_annual), TEAL), unsafe_allow_html=True)
        with pc3:
            st.markdown(
                plain_kpi_html("vs LEAP + InfoTrack",
                               "Baseline" if is_baseline else fmt_money(delta),
                               INK if is_baseline else (GREEN if delta >= 0 else RED)),
                unsafe_allow_html=True,
            )
        with pc4:
            cur_pct = 100
            sel_pct = round(sel_annual / baseline_annual * 100) if baseline_annual else 0
            st.markdown(
                f"""
                <div class="bar-row" style="margin-bottom:6px;">
                  <div class="bar-head"><span class="lbl">Current</span><span class="val">{fmt_money(baseline_annual)}</span></div>
                  <div class="bar-track"><div class="bar-fill" style="width:{cur_pct}%; background:#94a3b8;"></div></div>
                </div>
                <div class="bar-row" style="margin-bottom:0;">
                  <div class="bar-head"><span class="lbl">Selected</span><span class="val">{fmt_money(sel_annual)}</span></div>
                  <div class="bar-track"><div class="bar-fill" style="width:{min(sel_pct,100)}%; background:{TEAL};"></div></div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    with col_trend:
        with card():
            ...
            fig.update_layout(**plotly_base(210), hovermode="x unified", barmode="group", bargap=0.3, bargroupgap=0.15)
    ...
    with col_country:
        with card():
            ...
            st.plotly_chart(revenue_stream_fig(height=190), width="stretch",
                             config={"displayModeBar": False})

    with col_map:
        st.markdown('<div class="card-title" style="padding:0 2px; text-align:center;">Global Matter Footprint</div>',
                    unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            f'<b style="color:{INK};">{num_countries} countries</b> · '
            f'<b style="color:{INK};">{total_matters}</b> active matters</div>',
            unsafe_allow_html=True,
        )
...
def _locations_tab():
    ...
    st.plotly_chart(globe_fig, width="stretch",
                     config={"displayModeBar": False, "scrollZoom": False})
    st.markdown(
        f'<div style="text-align:center; font-size:12px; color:{MUTED}; margin-top:-6px;">'
        f'<b style="color:{INK};">{num_countries} countries</b> · '
        f'<b style="color:{INK};">{total_matters}</b> active matters</div>',
        unsafe_allow_html=True,
    )
...
st.caption("Build marker: v10 — extra bottom spacing after page content.")
