...
/* narrow windows: tighter nav buttons so labels are not cut off */
...
/* ---------- Decision Simulator highlight (static, no animation) ---------- */
.st-key-topbar .st-key-nav_Simulator div[data-testid="stButton"] button { background:linear-gradient(135deg,#fde68a,#f59e0b) !important; color:#3b2a00 !important; font-weight:800 !important;
    border:1px solid rgba(255,255,255,.7) !important; box-shadow:0 0 0 2px rgba(251,191,36,.40), 0 2px 6px rgba(15,23,42,.25) !important; }
.st-key-topbar .st-key-nav_Simulator div[data-testid="stButton"] button:hover { filter:brightness(1.06); color:#3b2a00 !important; }
.st-key-topbar .st-key-nav_Simulator div[data-testid="stButton"] button p { color:inherit !important; font-weight:800 !important; }
.st-key-topbar .st-key-nav_Simulator div[data-testid="stButton"] button[kind="primary"] { background:#ffffff !important; color:#b45309 !important; box-shadow:0 0 0 2px #f59e0b !important; }
.sim-hero { display:flex; align-items:center; justify-content:space-between; gap:18px; padding:14px 22px; border-radius:16px; color:#fff; margin:2px 0 10px 0;
    background:linear-gradient(100deg,#0a8496 0%,#2f7fd8 100%); box-shadow:0 10px 24px rgba(10,132,150,.28); }
.sim-hero-kicker { font-size:10.5px; font-weight:800; letter-spacing:1.4px; color:#fde68a; }
.sim-hero-title { font-size:22px; font-weight:800; line-height:1.15; }
.sim-hero-sub { font-size:12px; opacity:.92; margin-top:2px; max-width:720px; }
.sim-hero-tag { font-size:11px; font-weight:700; background:rgba(255,255,255,.18); border:1px solid rgba(255,255,255,.35); border-radius:999px; padding:4px 12px; white-space:nowrap; }
.sim-cta { font-size:12.5px; color:#78350f; background:#fffbeb; border:1px solid #fde68a; border-radius:12px; padding:8px 12px; }
.sim-cta-star { color:#f59e0b; font-weight:800; }
div[class*="st-key-act_"] button:disabled { opacity:.35 !important; cursor:not-allowed !important; background:#f8fafc !important; }
.goal-txt { font-size:12.5px; color:#0f172a; line-height:1.3; }
.goal-row { display:flex; align-items:center; gap:10px; padding:7px 4px; border-bottom:1px solid #f1f5f9; font-size:12.5px; }
.goal-row .lv { flex:0 0 190px; font-weight:600; color:#0f172a; } .goal-row .need { flex:1; color:#0f172a; }
</style>"""
