...
.kpi-card {{ background:{CARD}; border:1px solid {BORDER}; border-radius:14px; padding:12px 14px; height:100%; box-shadow: 0 4px 14px rgba(15,23,42,0.08), 0 1px 2px rgba(15,23,42,0.05); }}
...
.attorney-card {{ background:{CARD}; border:1px solid {BORDER}; border-radius:14px; padding:14px; text-align:center; box-shadow: 0 4px 14px rgba(15,23,42,0.08), 0 1px 2px rgba(15,23,42,0.05); }}
.attorney-avatar {{
...
.branch-card {{ background:{CARD}; border:1px solid {BORDER}; border-radius:14px; padding:14px; box-shadow: 0 4px 14px rgba(15,23,42,0.08), 0 1px 2px rgba(15,23,42,0.05); }}
...
def branch_globe_fig(height=230):
    ...
    fig.add_trace(go.Choropleth(
        locations=iso_codes, locationmode="ISO-3", z=country_counts.values,
        zmin=0, zmax=max(country_counts.max() * 1.3, 5),
        colorscale=[[0, "#bfe0e4"], [1, TEAL]], showscale=False,
...
def render_overview():
    ...
    spacer()
    col_trend, col_map, col_donut = st.columns([1.6, 0.9, 1.2])
    ...
    with col_trend:
        with st.container(border=True):
            ...
            fig.update_layout(**plotly_base(230), hovermode="x unified")
    ...
    with col_map:
        ...
        kicker, headline, desc = INSIGHTS[0]
        st.markdown(
            f"""
            <div style="padding:8px 2px 0 2px;">
              <div class="insight-kicker">{kicker}</div>
              <div class="insight-headline" style="font-size:12.5px;">{headline}</div>
    ...
    with st.container(border=True):
        ...
        with top2:
            st.markdown(
                ...
                f'International work now spans {pd.DataFrame(MATTERS)["country"].nunique()} countries</div>',
                unsafe_allow_html=True,
            )
        mdf = pd.DataFrame(MATTERS)
        ...
        layout_c = plotly_base(190)
        layout_c["margin"] = dict(l=8, r=16, t=4, b=4)
        fig_c.update_layout(**layout_c)
        fig_c.update_xaxes(showgrid=True, gridcolor="#f1f5f9")
        fig_c.update_yaxes(tickfont=dict(size=11, color=MUTED))
