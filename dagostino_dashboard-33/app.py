    ...
    with col_matrix:
        with card():
            st.markdown('<div class="card-title">Attorney Utilization — Hours by Practice Area</div>',
                        unsafe_allow_html=True)
            st.caption("Heatmap intensity + total hours, side by side. Darker = more hours in that area.")
            practice_cols = ["Corporate", "Litigation", "IP Portfolio", "Advisory"]
            attorney_names = [a["short"] for a in ATTORNEYS]
            z_matrix = [[a["hours"][c] for c in practice_cols] for a in ATTORNEYS]
            totals = [a["total"] for a in ATTORNEYS]
            ...
            fig_matrix.add_trace(go.Heatmap(
                z=z_matrix, x=practice_cols, y=attorney_names,
                colorscale=[[0, "#eaf5f7"], [0.5, "#7fd0d6"], [1, TEAL]],
                text=z_matrix, texttemplate="%{text}", textfont=dict(size=11, color=INK),
                hovertemplate="%{y} — %{x}: %{z} hrs<extra></extra>",
                showscale=False, xgap=3, ygap=3,
            ), row=1, col=1)
            fig_matrix.add_trace(go.Bar(
                x=totals, y=attorney_names, orientation="h",
                marker_color=TEAL, text=[f"{t}h" for t in totals], textposition="outside",
                hovertemplate="%{y}: %{x} hrs total<extra></extra>",
            ), row=1, col=2)

            fig_matrix.update_yaxes(autorange="reversed", row=1, col=1, tickfont=dict(size=11, color=INK))
            fig_matrix.update_yaxes(autorange="reversed", row=1, col=2, showticklabels=False)
            fig_matrix.update_xaxes(side="top", tickfont=dict(size=10, color=MUTED), row=1, col=1)
            fig_matrix.update_xaxes(visible=False, range=[0, max(totals) * 1.3], row=1, col=2)
            fig_matrix.update_layout(
                margin=dict(l=8, r=36, t=28, b=8), height=300,
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                showlegend=False, font=dict(family="Inter, sans-serif", color=MUTED, size=10),
            )
