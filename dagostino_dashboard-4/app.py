...
def branch_globe_fig(height=230):
    """Interactive orthographic 'globe' — only the country holding matters is
    highlighted, shaded by total matter volume, with branch pins sized and
    labelled by each branch's matter count. Plotly's orthographic geo
    projection supports native click-drag rotation."""
    bdf = pd.DataFrame(BRANCHES)
    total_matters = int(bdf["active_matters"].sum())

    fig = go.Figure()
    fig.add_trace(go.Choropleth(
        locations=["AUS"], locationmode="ISO-3", z=[total_matters],
        zmin=0, zmax=max(total_matters * 1.4, 100),
        colorscale=[[0, "#bfe0e4"], [1, TEAL]], showscale=False,
        marker_line_color="#ffffff", marker_line_width=0.6,
        customdata=[total_matters],
        hovertemplate="Australia — %{customdata} active matters<extra></extra>",
    ))
...
def render_overview():
    ...
    with col_map:
        st.markdown('<div class="card-title" style="padding:0 2px; text-align:center;">Branch Locations</div>',
                    unsafe_allow_html=True)
        globe_fig, total_matters = branch_globe_fig(height=230)
        st.plotly_chart(globe_fig, width="stretch",
                         config={"displayModeBar": False, "scrollZoom": False})
        st.markdown(
            f'<div style="text-align:center; font-size:11px; color:{MUTED};">'
            f'Australia — <b style="color:{INK};">{total_matters}</b> active matters '
            f'across 3 branches</div>',
            unsafe_allow_html=True,
        )
...
def _locations_tab():
    st.markdown('<div class="section-sub">3 branches — drag to rotate the globe, hover a pin, or check the cards below.</div>',
                unsafe_allow_html=True)
    globe_fig, total_matters = branch_globe_fig(height=340)
    st.plotly_chart(globe_fig, width="stretch",
                     config={"displayModeBar": False, "scrollZoom": False})
    st.markdown(
        f'<div style="text-align:center; font-size:12px; color:{MUTED}; margin-top:-6px;">'
        f'Australia — <b style="color:{INK};">{total_matters}</b> active matters across 3 branches</div>',
        unsafe_allow_html=True,
    )
