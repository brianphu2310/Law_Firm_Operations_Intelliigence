...
- **Frozen header** on the home dashboard: title, slicers, and the five core KPIs stay pinned
  to the top of the screen while everything else scrolls beneath them.
- **3D relationship network** (Relationships and Locations page): rotate, zoom, and click any
  node - or use the dropdown - to highlight its direct connections; click a newly-highlighted
  node to walk the chain (e.g. MATTER to TRUST ACCOUNT to COMMBIZ). An in-page "How to use this
  diagram" panel explains the controls.
- **Drill-down map**: click an office marker (or use the dropdown) to see that office's matter
  count, fee income, expenses, and top practice area.
- **Monthly financial trend** and **practice area profitability (estimated)** charts, plus a
  **cash-flow bridge waterfall** and a **year-over-year comparison** chart, all driven by the
  same slicers as the rest of the dashboard.
- **Location scorecard** with conditional colour formatting (green/red gradients).
