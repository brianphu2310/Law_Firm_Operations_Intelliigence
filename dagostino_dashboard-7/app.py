...
.stApp {{ background: {BG}; }}
.block-container {{ padding-top: 0.7rem; padding-bottom: 1.2rem; max-width: 1440px; }}
#MainMenu, footer, header[data-testid="stHeader"] {{ visibility: hidden; }}
div[data-testid="stVerticalBlock"] {{ gap: 0.45rem; }}

div[data-testid="stVerticalBlockBorderWrapper"] {{ border-radius: 18px !important; }}
div[data-testid="stVerticalBlockBorderWrapper"] > div {{
    background:{CARD}; border-radius: 18px !important;
    box-shadow: 0 6px 20px rgba(15,23,42,0.11), 0 1.5px 4px rgba(15,23,42,0.06);
...
.kpi-card {{ background:{CARD}; border:1px solid {BORDER}; border-radius:16px; padding:12px 14px; height:100%; box-shadow: 0 6px 18px rgba(15,23,42,0.10), 0 1.5px 3px rgba(15,23,42,0.05); }}
...
.attorney-card {{ background:{CARD}; border:1px solid {BORDER}; border-radius:16px; padding:14px; text-align:center; box-shadow: 0 6px 18px rgba(15,23,42,0.10), 0 1.5px 3px rgba(15,23,42,0.05); }}
.attorney-avatar {{
...
.branch-card {{ background:{CARD}; border:1px solid {BORDER}; border-radius:16px; padding:14px; box-shadow: 0 6px 18px rgba(15,23,42,0.10), 0 1.5px 3px rgba(15,23,42,0.05); }}
...
def matters_streamgraph_fig(height=210):
    """Streamgraph (silhouette-centered stacked area) of matter counts by
    country, layered by status — same flowing-river style as the reference
    'Price by Room Type and Payment Mode' chart, built with manually
    offset stacked area traces since Plotly has no native streamgraph type."""
    mdf = pd.DataFrame(MATTERS)
    pivot = mdf.pivot_table(index="country", columns="status", values="name",
                             aggfunc="count", fill_value=0)
    for s in ["Closed", "On Hold", "Open"]:
    ...
    baseline = -pivot["__total__"].values / 2.0
    layers = [
        ("Closed", "rgba(148,163,184,0.55)", "#94a3b8"),
        ("On Hold", "rgba(245,158,11,0.55)", AMBER),
        ("Open", "rgba(16,185,129,0.65)", GREEN),
    ]

    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=countries, y=baseline, mode="lines",
        line=dict(width=0, shape="spline"), hoverinfo="skip", showlegend=False,
    ))
    cum = baseline.copy()
    for status, fill_color, line_color in layers:
        cum = cum + pivot[status].values
        fig.add_trace(go.Scatter(
            x=countries, y=cum, mode="lines", line=dict(width=1, color=line_color, shape="spline"),
            fill="tonexty", fillcolor=fill_color, name=status,
            customdata=pivot[status].values,
            hovertemplate="%{x} — " + status + ": %{customdata} matter(s)<extra></extra>",
        ))
    fig.update_layout(
        margin=dict(l=8, r=8, t=4, b=4), height=height,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
...
def render_overview():
    ...
    with col_trend:
        with st.container(border=True):
            ...
            fig.add_trace(go.Scatter(x=TREND["month"], y=TREND["Litigation"], mode="lines",
                                      name="Litigation", line=dict(color=TEAL, width=2.2, shape="spline"),
                                      hovertemplate="%{x}: %{y} hrs<extra>Litigation</extra>"))
            fig.add_trace(go.Scatter(x=TREND["month"], y=TREND["Corporate"], mode="lines",
                                      name="Corporate", line=dict(color="#67c1cb", width=2.2, shape="spline"),
                                      hovertemplate="%{x}: %{y} hrs<extra>Corporate</extra>"))
            fig.update_layout(**plotly_base(210), hovermode="x unified")
            fig.update_xaxes(showgrid=False, tickfont=dict(size=8.5))
    ...
    with col_country:
        with st.container(border=True):
            st.markdown('<div class="card-title">Matters by Country &amp; Status</div>', unsafe_allow_html=True)
            st.markdown(
                f"""
                <div class="legend-row">
                  <span class="legend-item"><span class="legend-dot" style="background:{GREEN}"></span>Open</span>
                  <span class="legend-item"><span class="legend-dot" style="background:{AMBER}"></span>On Hold</span>
                  <span class="legend-item"><span class="legend-dot" style="background:#94a3b8"></span>Closed</span>
                </div>
                """,
                unsafe_allow_html=True,
            )
            st.plotly_chart(matters_streamgraph_fig(height=190), width="stretch",
